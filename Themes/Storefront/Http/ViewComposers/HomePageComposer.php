...
class HomePageComposer
{
    ...
    public function compose($view)
    {
        $view->with([
            ...
            'twoColBanners' => $this->twoColBanners(),
            'threeColBanners' => $this->threeColBanners(),
            'fourColBanners' => $this->fourColBanners(),
    ...
    {
        $categoryIds = Collection::times(16, function ($number) {
            if (! is_null(setting("storefront_featured_categories_section_category_{$number}_category_id"))) {
    ...
    private function productGrid()
    {
        ...
        if (! is_null(setting("storefront_product_grid_section_tab_1_product_type"))) {
            return [
                'title' => setting("storefront_product_grid_section_tab_1_title"),
                'subtitle' => setting("storefront_product_grid_section_tab_1_subtitle"),
                'products' => $this->getProductListFromSetting('storefront_product_grid_section_tab_1_products'),
            ];
        }
    }
    ...
    private function twoColBanners()
    {
        return DB::table('banner_two_col')->orderBy('id', 'desc')->get();
    }

    private function threeColBanners()
    {
        return DB::table('banner_three_col')->orderBy('id', 'desc')->get();
    }

    private function fourColBanners()
    {
        return DB::table('banner_four_col')->orderBy('id', 'desc')->get();
    }

    private function bannerSettings() {
        return DB::table('banner_settings')->where('id', '=', '1')->first();
    }

    private function getProductListFromSetting($key)
    {
        return Product::listForCard(setting($key, []));
    }
}
