...
class SidebarExtender extends BaseSidebarExtender
{
    public function extend(Menu $menu)
    {
        $menu->group(trans('admin::sidebar.content'), function (Group $group) {
            ...
            $group->item(trans('Pc Builder'), function (Item $item) {
                $item->icon('fa fa-laptop');
                $item->route('admin.pcbuilder.index');
                $item->isActiveWhen(route('admin.dashboard.index', null, false));
            });
            $group->item(trans('Banner'), function (Item $item) {
                $item->icon('fa fa-paint-brush');
                $item->route('admin.banners.twocol.index');
                $item->isActiveWhen(route('admin.dashboard.index', null, false));
            });
        });
