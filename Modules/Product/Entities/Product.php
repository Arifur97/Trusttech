...
class Product extends Model
{
     ...
     */
    protected $appends = [
        'category_image', 'formatted_price', 'is_in_stock',
    ...
    public static function listForCard($ids = [])
    {
        return static::select('id')
            ->withName()
            ->withCategoryImage()
            ...
            ->addSelect([
                ...
                'products.offer_price_start',
                'products.offer_price_end',
            ])
            ->get();
    }
    ...
    {
        $query->with('translations:id,product_id,locale,name,short_description');
    }
    ...
    public function scopeWithCategoryImage($query)
    {
        $query->with(['files' => function ($q) {
            $q->wherePivot('zone', 'category_image');
        }]);
    }
    ...
    /**
     * Get the product's category image.
     *
    ...
    public function getDiscountOfferAttribute()
    {
        if (is_null($this->offer_message)) {
            return false;
        }

        if ($this->hasDiscountOfferStartDate() && $this->hasDiscountOfferEndDate()) {
            return $this->discountOfferStartDateIsValid() && $this->discountOfferEndDateIsValid();
        }

        return false;
    }

    private function hasDiscountOfferStartDate()
    {
        return ! is_null($this->offer_price_start);
    }

    private function hasDiscountOfferEndDate()
    {
        return ! is_null($this->offer_price_end);
    }

    private function discountOfferStartDateIsValid()
    {
        return today() >= $this->offer_price_start;
    }

    private function discountOfferEndDateIsValid()
    {
        return today() <= $this->offer_price_end;
    }
