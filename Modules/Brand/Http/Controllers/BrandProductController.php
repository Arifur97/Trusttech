...
class BrandProductController
{
    ...
    public function index($slug, Product $model, ProductFilter $productFilter)
    {
        ...
        return view('public.products.index', [
            ...
            'dataMetaTitle' => $brand->meta->meta_title ?? '',
            'dataMetaDesc' => $brand->meta->meta_description ?? '',
            'dataKeyword' => $brand->meta->meta_keywords ?? '',
            'dataDescription' => $brand->meta->description ?? '',
            'dataMetaSchema' => $brand->meta->meta_schema ?? '',
        ]);
    }
}
